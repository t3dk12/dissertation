...
def fmessage_inv(bd,X,m,layer=5):
    ...
    '''
    o   =  []                         
    o.append(tf.sigmoid(tf.matmul(X,m['fmwi_'+bd],name='mg_input')+m['fmbi_'+bd]))   # input layer
      ...
      if self.energy_term['eang']:
         ...
         if self.ang_clip:                        # regularize pi term
            for ang in self.ang_clip: 
                if self.nang[ang]>0:
                   fang = tf.where(tf.greater_equal(self.thet2[ang],self.ang_clip[ang]),1.0,0.0)  
